...
def main():
	with open("settings.json", "r") as f:
		data = load(f)
	ui.newData()
	choice = input(f"{color.okgreen}Nhập lựa chọn: {color.reset}")
	if choice == "0":
		pass
	elif choice == "1":
		t(data, True)
		...
		ui.slowPrinting(f"{color.fail}[INFO] {color.reset}Lựa chọn không có, vui lòng chọn lại")
def t(data, all):
 data['token'] = input("Vui lòng nhập token account: ")
